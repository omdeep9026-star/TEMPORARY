...
def sft_data_generator_bos_bestfit(split, buffer_size=100):
    ...
    Conversations are packed using best-fit algorithm. Conversations longer than
    a row are skipped; remaining row space is padded instead of cropping.
    ...
    while True:
        ...
        if not any(any(mask_row[1:]) for mask_row in mask_rows):
            print0("Skipping SFT batch with no supervised targets")
            continue
